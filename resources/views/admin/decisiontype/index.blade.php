...
<body>
    <div class="d-flex justify-content-end">
        <button  class="btn btn-dark" onclick="window.location.href='{{route('decisiontype.create')}}'">Create</button>
    </div>
    <div class="d-flex justify-content-center"><form class="form-group" method="POST" action="{{route('decisiontype.serach')}}">
...
@foreach($all_decisions as $s)
    ...
    <table  class="table table-striped">
        <th class="table-secondary d-flex justify-content-start" scope="col"><button  class="btn btn-primary" onclick="window.location.href='{{route('decisiontype.edit',$s->id)}}'">Edit</button></th>
        <th class="d-flex justify-content-center table-active" scope="col">Name: {{$s->name}}</th>


        <th class="table-success d-flex justify-content-end" scope="col"><form action="{{route('decisiontype.destroy',$s->id)}}" method="post">
            @csrf
            @method('delete')
